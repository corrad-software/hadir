...
import { Tag, Plus, Check, X, Pencil, Trash2, Search, ChevronLeft, ChevronRight } from "lucide-vue-next";
import AdminLayout from "@/layouts/AdminLayout.vue";
import AppBreadcrumb from "@/components/AppBreadcrumb.vue";
import { useToast } from "@/composables/useToast";
import { useConfirmDialog } from "@/composables/useConfirmDialog";
import { listJobTitles, createJobTitle, updateJobTitle, deleteJobTitle } from "@/api/cms";
...
async function load() {
  ...
  try {
    ...
    const res = await listJobTitles(`?${params}`);
    titles.value = res.data;
    total.value = Number(res.meta?.total ?? 0);
    totalPages.value = Number(res.meta?.totalPages ?? 1);
  } catch (e) {
    toast.error("Failed to load job titles", e instanceof Error ? e.message : "");
  } finally {
    loading.value = false;
  }
}
...
async function saveEdit(id: number) {
  ...
  saving.value = true;
  try {
    await updateJobTitle(id, { name: editName.value.trim() });
    toast.success("Saved");
    cancelEdit();
    await load();
  } catch (e) {
...
async function saveNew() {
  ...
  saving.value = true;
  try {
    await createJobTitle({ name: newName.value.trim() });
...
async function remove(t: JobTitle) {
  const ok = await confirmDialog.confirm({
    title: "Delete job title?",
    message: `"${t.name}" will be permanently removed. Employees currently assigned this title will become unassigned.`,
    confirmText: "Delete",
    destructive: true,
  });
  if (!ok) return;
  try {
    await deleteJobTitle(t.id);
  ...
  <AdminLayout>
    <div class="mx-auto max-w-7xl space-y-4">
      ...
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 class="page-title">Job Titles</h1>
          <p class="mt-0.5 text-sm text-slate-500">Official job title options available when editing an employee profile.</p>
        </div>
      ...
      <article class="rounded-lg border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center gap-2 border-b border-slate-100 px-4 py-2.5">
          ...
          <h2 class="text-sm font-semibold text-slate-900">All Job Titles</h2>
        ...
        <div v-else class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
              ...
              <tr v-if="titles.length === 0 && !addingNew">
                <td colspan="2" class="px-4 py-8 text-center text-sm text-slate-400">No job titles found.</td>
              </tr>

              <tr v-for="t in titles" :key="t.id" class="transition-colors hover:bg-slate-50">
                <td class="px-4 py-2.5">
                  ...
                  />
                  <span v-else class="font-medium text-slate-900">{{ t.name }}</span>
                </td>
                <td class="px-4 py-2.5 text-right">
                  <div v-if="editingId === t.id" class="flex items-center justify-end gap-1">
                    <button class="flex h-8 w-8 items-center justify-center rounded-lg text-green-600 transition-colors hover:bg-green-50 disabled:opacity-40" :disabled="saving || !editName.trim()" @click="saveEdit(t.id)"><Check class="h-4 w-4" /></button>
                  ...
                  <div v-else class="flex items-center justify-end gap-1">
                    <button class="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700" title="Edit" @click="startEdit(t)"><Pencil class="h-3.5 w-3.5" /></button>
                    <button class="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-rose-50 hover:text-rose-600" title="Delete" @click="remove(t)"><Trash2 class="h-3.5 w-3.5" /></button>
                  </div>
                </td>
              </tr>
            </tbody>
